...
function EditPermission() {
  ...
  let formik = useFormik({
    ...
    onSubmit: values => {
      HttpService.put(apiScheme.permission.planners+`/${id}`, values)
      .then(({data}) => {
        Swal.fire({
          title : 'Success',
          text: data.message,
  ...
  useEffect(() => {
    HttpService.get(`${apiScheme.permission.planners}/${id}`)
    .then(({data}) => {
      setPermission(data);
  ...
  return (
    <div>
      <p className="lead">
        ...
        <span>Edit permission {permission ? permission.user.name : ''}</span>
      </p>
      <hr />
      <div>
